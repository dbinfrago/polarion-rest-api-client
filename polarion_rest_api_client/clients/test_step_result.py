...
class TestStepResults(bc.SingleGetClient[dm.TestStepResult], bc.UpdateClient[dm.TestStepResult], bc.CreateClient[dm.TestStepResult], bc.MultiGetClient[dm.TestStepResult]):
    ...
    def get(self, to_get: dm.TestStepResult) -> dm.TestStepResult:
        ...
        testStep_Result = dm.TestStepResult(test_case_id=to_get.test_case_id,
                                            ...
                                            comment=(
                                                dm.TextContent(
                                                    str(data.attributes.comment.type_),
                                                    data.attributes.comment.value if data.attributes.comment.value is not oa_types.UNSET else None  
                                                )
    ...
    async def async_get(self, to_get: dm.TestStepResult) -> dm.TestStepResult | None:
        """Async Getting a Test Step Result for a test case"""
    ...
    async def _async_update(self, to_update: list[dm.TestStepResult]) -> None:
        """Async Updating a Test Step Result for a test case"""
    ...
    async def _async_create(self, to_update: list[dm.TestStepResult]) -> None:
        """Async Creating Test Step Results for a test case"""
        body_data = self._build_post_request_data(to_update)
        body = api_models.TeststepResultsListPostRequest(data=body_data)
        ...
        if (isinstance(parsed_response, api_models.TeststepResultsListGetResponse)
            ...
            for item in parsed_response.data:
                ...
                testStep_Result = dm.TestStepResult(test_case_id=items[0].test_case_id,
                                            ...
                                            test_run_id=items[0].test_run_id,
                                            test_case_project_id=items[0].test_case_project_id,
                                            result=attributes.result if attributes.result is not oa_types.UNSET else None,
                                            comment=(
                                                dm.TextContent(
                                                    str(attributes.comment.type_),
                                                    attributes.comment.value if attributes.comment.value is not oa_types.UNSET else None
                                                )
    ...
    async def async_get_multi(self, 
                  items: list[dm.TestStepResult],
                  page_size: int = 100,
                  page_number: int = 1,
                  fields: dict[str, str] | None = None,
                  revision: str | None = None,) -> t.Tuple[list[dm.TestStepResult], bool]:
        """Async Getting multiple Test Step Results for a test case"""
        if fields is None:
            fields = self._client.default_fields.teststep_results
        sparse_fields = self._build_sparse_fields(fields)
    ...
    def _fill_teststep_result_attributes(
        ...
        return attributes_type(
            result=teststep_Result.result if teststep_Result.result is not None else oa_types.UNSET,
            comment=comment_item_cls(
                type_=comment_item_type,
                value=comment_value,
            ) if comment_value is not None else oa_types.UNSET,
        )


    def _build_patch_request_data(
        self, stepResult: dm.TestStepResult
    ) -> api_models.TeststepResultsListPatchRequestDataItem:
